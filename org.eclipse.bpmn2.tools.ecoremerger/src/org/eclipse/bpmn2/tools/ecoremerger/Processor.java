...
public class Processor {
	...
	private void applyExtendedMetadata(EModelElement target, EModelElement source) {
		...
		if (extendedMetadata != null) {
			LOGGER.info("Patching ExtendedMetadata from " + getName(source) + " to " + getName(target));
	...
	private EStructuralFeature getCorrespondingFeature(EClass mofClass, EStructuralFeature xsdFeature, ClassMappingException exception) {
		EStructuralFeature result = null;
		if (mofClass != null) {
			String mofFeatureName = xsdFeature.getName();
			if (exception != null) {
				for (AttributeMappingExcpetion ex : exception.getAttributeExceptions()) {
					if (ex.getXsdAttributeName().equals(xsdFeature.getName())) {
			...
			if (result == null) {
				...
				if ("id".equals(xsdFeature.getName())) {
					...
					LOGGER.severe("No corresponding field found for " + xsdFeature.getEContainingClass().getName() + "."
	...
	private ClassMappingException getExceptionForXsdClass(EClass xsdClass) {
	...
	private EClass getMofClass(EClass xsdClass, ClassMappingException exception) {
	...
	private EClass getClass(EPackage ePackage, String name) {
	...
	private EClass getMofClass(String name) {
		for (EObject eObject : getMofContent()) {
	...
	public List<EClass> getXsdContent(boolean includeDocRoot) {
		ArrayList<EClass> result = new ArrayList<EClass>();
	...
	private void importDocumentRoot(EPackage mofPackage, EPackage xsdPackage) {
		...
		LOGGER.info("<H1>Processing Global Elements - Importing DocumentRoot for " + mofPackage.getName() + "</H1>");
		EClass oldDocumentRoot = getClass(mofPackage, "DocumentRoot");
		if (oldDocumentRoot != null) {
			LOGGER.warning("Deleting old DocumentRoot");
			EcoreUtil.delete(oldDocumentRoot);
		}
		EClass documentRoot = getClass(xsdPackage, "DocumentRoot");
		...
		for (EStructuralFeature feature : documentRoot.getEStructuralFeatures()) {
			...
			if (feature.getEType() instanceof EClass) {
				EClass newEClass = getMofClass((EClass) feature.getEType(), null);
				if (newEClass != null) {
					...
					LOGGER.info("Changing DocumentRoot." + feature.getName() + " to " + newEClass.getName());
	...
	private void importXsdComplexTypesMetadata() {
		...
		for (EClass xsdClass : getXsdContent(false)) {
			LOGGER.info("<H3>Processing " + xsdClass.getName() + "</H3>");

			ClassMappingException exception = getExceptionForXsdClass(xsdClass);
			EClass mofClass = getMofClass(xsdClass, exception);
			if (mofClass != null) {

				applyExtendedMetadata(mofClass, xsdClass);

				for (EStructuralFeature xsdFeature : xsdClass.getEStructuralFeatures()) {
					EStructuralFeature mofFeature = getCorrespondingFeature(mofClass, xsdFeature, exception);
					if (mofFeature != null) {
						...
						if (mofFeature instanceof EReference) {
							...
							if (!mofReference.isContainer() && !mofReference.isContainment()) {
								if ("IDREF".equals(xsdFeature.getEType().getName())) {
									...
									LOGGER.warning(mofClass.getName() + "." + mofFeature.getName()
											+ " reference corresponds to neither IDREF nor QName, but " + xsdFeature.getEType().getName());
								}
							}
						}
	...
	private void fixModelFlags() {
		...
		for (EClass mofClass : getMofContent()) {

			// Sort features as in original
			ECollections.sort(mofClass.getEStructuralFeatures(), new Comparator<EStructuralFeature>() {
			...
			for (EStructuralFeature feature : mofClass.getEStructuralFeatures()) {
				...
				Boolean hasNoExtendedMetadata = feature.getEAnnotation(ExtendedMetaData.ANNOTATION_URI) == null;
				...
				if (feature instanceof EReference) {
					...
					if (feature.isDerived() != hasNoExtendedMetadata) {
						LOGGER.severe(featureName + " has inconsistent derived flag. Correcting.");
						feature.setDerived(hasNoExtendedMetadata);
					}
					if (feature.isTransient() != hasNoExtendedMetadata) {
						LOGGER.severe(featureName + " has inconsistent transient flag. Correcting.");
